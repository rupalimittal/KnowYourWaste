...
var writePath = 'C:/Users/rupali mittal';
    ...
    connection.query('SELECT * FROM waste_managment WHERE userid = ?', [userid], function (error, results, fields) {
            if (error) {
              ...
              if (results.length > 0) {
                ...
                connection.query('UPDATE waste_managment SET printCount = ? WHERE userid = ?', [printCount, userid], function (error, results, fields) {
                  if (error) {
